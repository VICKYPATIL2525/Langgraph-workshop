...
Workflow Visualization:
    START → [Outline Generator] → [Content Generator] → END
        ↓            ↓                     ↓              ↓
...
"""
Edge Definition:
- START → outline_generator: Begin with outline generation
- outline_generator → content_generator: After outline, generate content
- content_generator → END: After content, workflow is complete
...
print(f"\n📋 OUTLINE ({len(final_state['outline'])} characters):")
print("-" * 40)
print(final_state['outline'])
print("-" * 40)

print(f"\n📄 CONTENT ({len(final_state['content'])} characters):")
print("-" * 40)
...
This pattern can be extended for more complex content creation:
  Title → Outline → Research → Draft → Edit → Final
"""